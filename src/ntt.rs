...
pub fn butterfly_inv(a: i16, b: i16, zeta: i16) -> (i16, i16) {
    let ap = barrett_reduce(a + b); //[0, 3329[
    let bp = fqmul(zeta, a - b); //[-1664, 1664]
    (ap, bp)
}

pub fn butterfly(a: i16, b: i16, zeta: i16) -> (i16, i16) {
    let t = fqmul(zeta, b); //[-1664, 1664]
    let bp = a - t;
    let ap = a + t;
    (ap, bp)
}
...
pub fn ntt(r: &mut [i16; 256]) {
    ...
    while len >= 2 {
        ...
        while start < 256 {
            ...
            let mut j = start;
            while j < start + len {
                let (newj, newj_len) = butterfly(r[j], r[j + len], zeta);
                r[j] = newj;
                r[j + len] = newj_len;
                j += 1;
            }
...
pub fn ntt_inv(r: &mut [i16; 256]) {
    let mut len = 2 as usize;
    let mut k = 0;
    while len <= 128 {
        let mut start = 0;
        while start < 256 {
            ...
            let mut j = start;
            while j < start + len {
                let (newj, newj_len) = butterfly_inv(r[j], r[j + len], zeta);
                if r[j] > 3329
                    || r[j + len] > 3329
                    || newj > 3329
                    || newj_len > 3329
                    ...
                    || newj_len < -3329
                {
                    println!("{}, {}, {}, {}", r[j], r[j + len], newj, newj_len);
                }
                r[j] = newj;
                r[j + len] = newj_len;
                j += 1;
